...
class QueueManager:
    def __init__(self, config, analyzer):
        ...
        # буфер для генератора и основная очередь проверенных транзакций
        self.temp_buffer = []
        self.queue = []
        self.total_processed = 0

        # берем параметры шлюзов из конфигурации
        base_mu = self.settings["gateway_base_mu"]
        gateways_count = self.settings.get("gateways_count", 3)

        # создаем список серверов-шлюзов
        self.gateways = []
        for i in range(gateways_count):
            self.gateways.append(Gateway(gateway_id=i, base_mu=base_mu))

    def update(self, dt):
        # 1. забираем транзакции из буфера генератора и проверяем через антифрод
        for tx in self.temp_buffer:
            if self.analyzer.check_is_safe(tx):
                self.queue.append(tx)
        self.temp_buffer.clear()

        # 2. обновляем состояние шлюзов и считаем случайные поломки
        for gw in self.gateways:
            if not gw.is_broken:
                # шанс поломки привязан к прошедшему времени dt
                ...
                if random.random() < break_prob:
                    ...
                    gw.current_transaction = None  # транзакция теряется при поломке

            # запускаем внутренний таймер обработки или ремонта в шлюзе
            is_done = gw.update(dt)
            if is_done:
                self.total_processed += 1

            # 3. если шлюз ничем не занят и не сломан, даем ему новую задачу из очереди
